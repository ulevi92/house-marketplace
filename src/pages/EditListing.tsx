...
interface EditListingState extends ListingStateType {
  listing?: FormDataType;
}

const EditListing = () => {
  const [state, setState] = useState<EditListingState>({
  ...
  const { loading, formData, geolocationEnabled, listing } = state;
  const {
    type,
    name,
    bedrooms,
  ...
  useEffect(() => {
    const getListing = async () => {
      ...
      if (!docSnap.exists()) {
        ...
        toast.info(`This listing isn't exists`);
      }
    };

    if (isMounted.current) getListing();

    return () => {
      isMounted.current = false;
    };
  }, [formData, navigate, params.listingId]);
